...
if (form) {
  form.addEventListener("submit", event => {
    ...
    if (!title) {
      setStatus("Task can not be empty", true);
      return;
    }
    ...
    fetch(`${API_BASE}/add.php?stdid=${STUDENT_ID}&key=${API_KEY}`,{
     ...
     .then(data => {
      ...
      if (data.task) {
        ...
        setStatus("Error: Failed adding task", true);
      }
      else {
        setStatus ("Strange Error",true);
      }
    })
...
/**
 * TODO 3:
 * For each task that you render, screate a "Delete" button.
 * When clicked:
 *   - send a request to:
...
function renderTask(task) {
  ...
  deleteButton.style.padding = "6px 14px";
